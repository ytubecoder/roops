...
# four live ids.
ID_RE = re.compile(r"^ADG-(?:[A-Z]+-)?(\d{2,})$")
HEAD_RE = re.compile(r"^##\s+(?:~~)?\s*(ADG-(?:[A-Z]+-)?\d+)\b")
...
next_id = f"ADG-<SRC>-{high_water + 1:02d}"
...
print("- prior legacy single-part ids (ADG-NN) are carried VERBATIM — never renamed.")
