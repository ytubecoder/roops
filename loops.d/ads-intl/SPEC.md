...
`ads-intl:<ADI-NN>`. The `ADI-NN` id is the durable per-exception identity:
...
so the loop keeps re-emitting `ads-intl:ADI-NN` every run while it is still
...
- **ID pattern `^ADI-\d{2,}$`** — two-or-more digits, because a daily loop
...
- **IDs are NEVER reused after a strike.** A new id is always (max id ever seen)
  + 1; the first run with no prior set starts at `ADI-01`.
