...
This loop is the scheduled check-in for the Google INTERNATIONAL geo screen
(the `g-intl` experiment). It reads the Growth Console ads JSON surface,
surfaces the exceptions a human should act on (CPC vs the scan band, geo
delivery, budget-guard headroom, paused-probe state), and distils them into a
per-run **action set** of read-only, context-linked briefs (`ADI-NN`). It
...
exceptions, assigns/keeps `ADI-NN` ids, writes suggested orders with full
...
4. Cadence
`daily:18:25` (Manila local = ~06:25 EDT), SECOND in the morning stagger
...
5. Scope & exclusions
**In scope:** the intl google experiments ONLY — derived at run time from the
`/api/ads/campaigns` registry (the `g-intl` card, plus any future card whose
utm_campaigns contain "intl"). Today: g9–g12; google-intl-en-jul26 (ENABLED,
13 geos) + google-ja-jul26 / google-ko-jul26 (PAUSED — the native-script probe
failed and was killed; reviving them needs new evidence and Generalissimo's call). Intl
is a geo screen — market varies, message constant — never merged into the
message test.
**Explicitly excluded:** every non-intl google card (g-msg, g-theme — owned by
`ads-google`); X / Reddit networks; the `retired` card; any WRITE
(record_and_apply, ads API, git, CDP/browser). No cross-network/program checks
...
after 2026-07-28, legacy `ADI-NN` for carried ones. The action id is the durable
...
- **ID pattern `^ADI-(?:(?:EV|CMP|JRN|BUD|INP)-)?\d{2,}$`** — new ids are
  two-part `ADI-<SRC>-NN` (source = provenance designator; the number is ONE
  ...
  their own prefix (`ADG-` google, `ADR-` reddit, `ADX-` x, `ADP-` program — ADP's allowed sources are `PRG|BUD|INP`).
- **IDs are NEVER reused after a strike.** A new id's number is always (max
  ...
  genuinely-first run starts at `ADI-<SRC>-01`. Bare single-part ids are never
  minted anew.
...
- 🚨 **A run-integrity failure must ALSO be emitted as a finding with
  ...
  supervised run (`20260726T190729Z-ads-intl-58e835`): the engine correctly
