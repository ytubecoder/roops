...
Checks (stdlib only, per the harness ground rules):
  ...
  3. Every register heading id matches the two-part grammar
     ^ADI-(?:(?:EV|CMP|JRN|BUD|INP)-)?\\d{2,}$ (legacy single-part ids stay
...
def validate(set_dir: Path, continuity: dict | None = None) -> list[str]:
    ...
    # ONE number sequence per loop, shared across sources: ADI-CMP-08 and
    # ADI-JRN-08 are the same slot, not two actions.
