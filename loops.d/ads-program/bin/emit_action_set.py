#!/usr/bin/env python3
"""emit_action_set.py — write the ads-program action set into the run dir.

Reads ONE action-set payload from stdin and materializes it as
    <out-dir>/action-set/ACTIONS.md          (register, DMP shape)
    <out-dir>/action-set/actions/<ADP-NN>.md (one framing brief per action)
...
PAYLOAD FORMATS (auto-detected on the first non-space character):
  1. FLAT sectioned format (the ENGINE's format — REQUIRED in the engine
     ...
     default` (probe pair, allowlist `Bash(python3 <script>:*)`):
       DENIED : python3 s.py <<'EOF' / {"id": "ADP-01", "t": "x"} / EOF
       ALLOWED: python3 s.py <<'ACTIONSET' / id: ADP-01 / title: … / ACTIONSET
     ...
     NEW action ids use the two-part grammar `ADP-<SRC>-NN` (SRC one of
     EV/CMP/JRN/BUD/INP — the provenance of the finding); the single-part
     legacy shape `ADP-NN` is accepted only for ids carried forward from the
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        sys.stderr.write(
            ...
            "validator REMEDY: no ADP- id enters findings without a durable "
            "set behind it)\n"
        )
        return 1
