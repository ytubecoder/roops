#!/usr/bin/env python3
"""validate_action_set.py — validate a produced ads-program action set.

Ships with the ads-program loop and is run INSIDE the engine session via the
...
Checks (stdlib only, per the harness ground rules):
  ...
  3. Every register heading id matches ^ADP-\\d{2,}$ (two-or-more digits;
...
ID_RE = re.compile(r"^ADP-\d{2,}$")
# Register heading: "## ADP-07 — title"  (open)  or  "## ~~ADP-07 — title~~" (struck)
HEADING_RE = re.compile(r"^##\s+(~~)?\s*(ADP-\d+)\s+[—-]\s+(.*?)(~~)?\s*$")
    ...
    for aid in register_ids:
        ...
        if int(m.group(1)) <= high_water:
            errors.append(
                ...
                f"ADP-{high_water + 1:02d}. Ids are never reused, even after a strike."
            )
...
def validate(set_dir: Path, continuity: dict | None = None) -> list[str]:
    ...
    for line in reg_text.splitlines():
        ...
        if not ID_RE.match(aid):
            errors.append(f"ACTIONS.md: id {aid!r} does not match ^ADP-\\d{{2,}}$")
    ...
    if not register_ids:
        ...
        if not declared_empty:
            errors.append(
                "ACTIONS.md: no `## ADP-NN — title` register headings found "
    ...
    if briefs_dir.is_dir():
        for p in sorted(briefs_dir.glob("*.md")):
            ...
            if not ID_RE.match(bid):
                errors.append(f"actions/{p.name}: id does not match ^ADP-\\d{{2,}}$")
...
FAILURE_PROTOCOL = (
    ...
    "(findings: []). No ADP- id may enter the findings list without a durable "
