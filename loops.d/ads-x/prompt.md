...
Each exception becomes one **action** with a stable id `ADX-NN` (two-or-more
...
The script writes `action-set/ACTIONS.md`, `action-set/actions/ADX-NN.md`, and
...
- `findings`: one finding per **OPEN** action (skip struck ones), with
  `finding_id` = `ads-x:ADX-NN`, `title` = the action title, `severity`,
...
`ads-x:<ADX-NN>` — the durable per-action id, stable across runs for the
