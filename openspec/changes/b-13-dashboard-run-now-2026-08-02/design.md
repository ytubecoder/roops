...
3. **Worker is a dumb executor.** `threading.Thread(daemon=True)`; the whole body in
   ...
   here). After the subprocess returns: `_regen_dashboard(root)` best-effort, warn on
   stderr, never alters the recorded outcome.
...
(none — kanji choice 走 is presentation-only and swappable at review)
