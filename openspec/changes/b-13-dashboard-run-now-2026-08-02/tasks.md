...
- [ ] 1.1 Add the module-level job slot: `threading.Lock` + status dict
...
- [ ] 1.2 Add the worker function: daemon thread body in `try/finally`; calls
...
- [ ] 1.3 Route `POST /api/loops/<name>/run` in `handle_request`: name regex
...
- [ ] 1.4 Route `GET /api/run/status`: 200 + status dict; terminal snapshot persists
      until the next job starts.
- [ ] 1.5 Confirm §13.1 origin gate covers both routes with zero changes (it wraps
      `_do` for every request) — add a test, not code, if true.

## 2. Console tests (`tests/test_console.py`, hermetic)

- [ ] 2.1 Endpoint tests via `handle_request()` + `LoopsRoot`: 404 unknown loop,
...
- [ ] 2.2 Worker tests with `console._loopctl` monkeypatched (no engine ever runs):
...
- [ ] 2.3 Origin-gate test: non-loopback Host / wrong Content-Type on the run POST
      → 403, no worker started.

## 3. Dashboard control (`dashboard/generate.py`)

- [ ] 3.1 Render the `con-run` button (走, aria-label "run <name> now") inside the
      existing `.con-cell` for EVERY loop row — no installed gating, inert + hidden
      like its siblings.
- [ ] 3.2 Hydration script: after `api/state` success, fetch `api/run/status` once;
      if running, disable all 走 buttons and mark the running loop's.
- [ ] 3.3 Click handler: POST via the existing `post()` normalizer; on 202 poll
...
- [ ] 3.4 CSS for the button + its in-flight state in the generated stylesheet
      (kit-consistent, no new remote references).
- [ ] 3.5 Dashboard tests: control renders for installed AND non-installed loops;
      hidden by default; `tests/html_selfcontained.py` still passes.

## 4. Contract + docs

- [ ] 4.1 Amend `docs/INTERFACES.md` §13: two new rows in the endpoint table + new
      ...
      as the code.
- [ ] 4.2 Note the new surface in `docs/ADS_LOOPS_FOLLOWUP_WARMSTART.md` (phase-1
